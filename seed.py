...
def seed(db_url: str = "sqlite:///app.db") -> None:
    ...
    with SessionLocal() as db:
        ...
        for i in range(1, 101):
            ...
            db.add(
                Machine(
                    name=f"TM-{i:03d}",
                    machine_type=mtype,
                    category=random.choice(MACHINE_CATEGORIES),
                    status=(
                        "out_of_service"
                        if random.random() < OUT_OF_SERVICE_RATE
                        else "available"
                    ),
                    site_id=site.id,
        ...
        # Assignments
        #
        # Design: three assignments are created to cover the typical status
        # lifecycle (active, completed, cancelled).  Each assignment is owned
        # by the standard user and has the approver user listed as an
